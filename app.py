...
def main():
    ...
    st.markdown("Predict churn likelihood for telecom customers using machine learning.")

    add_selectbox = st.sidebar.selectbox("Select Prediction Mode", ('Online Prediction', 'Batch Prediction'))

    # Sidebar branding
    st.sidebar.markdown("---")
    st.sidebar.markdown("Created by [Your Name] · Connect on [LinkedIn](https://www.linkedin.com)")
    ...
    if add_selectbox == 'Online Prediction':
        ...
        with col1:
            ...
            OnlineSecurity = st.selectbox('Online Security', [2, 1, 0], format_func=lambda x: {2: 'Yes', 1: 'No internet service', 0: 'No'}[x])
            OnlineBackup = st.selectbox('Online Backup', [2, 1, 0], format_func=lambda x: {2: 'Yes', 1: 'No internet service', 0: 'No'}[x])

        with col2:
            TechSupport = st.selectbox('Tech Support', [2, 1, 0], format_func=lambda x: {2: 'Yes', 1: 'No internet service', 0: 'No'}[x])
            Contract = st.selectbox('Contract Type', [2, 1, 0], format_func=lambda x: {2: 'Two Year', 1: 'One Year', 0: 'Month-to-Month'}[x])
            MonthlyCharges = st.number_input('Monthly Charges ($)', min_value=18, max_value=120, value=40)
            TotalCharges = st.number_input('Total Charges ($)', min_value=18, max_value=9000, value=400)
        ...
        if st.button("Predict Churn"):
            ...
            output = "🔴 Customer is likely to Churn" if prediction == 1 else "🟢 Customer is not likely to Churn"
            ...
            st.plotly_chart(px.bar(df_importance, x='Feature', y='Importance', color='Importance'))
        ...
        if uploaded_file is not None:
            ...
            href = f'<a href="data:file/csv;base64,{b64}" download="batch_predictions.csv">Download Results CSV</a>'
